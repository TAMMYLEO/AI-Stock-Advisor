...
if "chat_messages" not in st.session_state: 
    st.session_state.chat_messages = [{"role": "assistant", "content": "您好呀！我是您的專屬 AI 交易顧問。我們現在不僅會幫您看均線，我也會幫您注意財報、法說會，甚至是填息的狀況喔！"}]
...
with st.sidebar:
    ...
    if selected_teacher_name == "💡 自訂大師 (手動輸入)":
        selected_logic = st.text_area("自訂邏輯：", value=f"看重財報與法說會轉機，站上 {short_ma}MA 加碼，極度重視填息機率與估值 (PEG)。")
...
with tab_pk:
    ...
    if num_stocks > 0:
        ...
        user_news = st.text_area("貼上您看到的利多新聞、法說會大餅或小道消息，大師將化身「毒舌查核員」為您測謊：", placeholder="例如：群創2026下半年車載面板訂單滿載，產能供不應求，法人預估營收將創高...")
                ...
                with st.spinner("大師正在交叉比對新聞真偽與量價數據，請稍候一下下..."):
                    try:
                        ...
                        for i, res in enumerate(results):
                            ...
                            data_strings.append(f"【標的 {i+1} ({d['display_name']})】：收盤:{d['current_price']:.2f} | PE:{d['pe_ratio']:.2f} | PEG:{d['peg_ratio']:.2f} | 殖利率:{d['div_yield']:.2f}% | 填息機率:{d['fill_rate']:.0f}% (平均{d['avg_fill_days']:.0f}天) | {short_ma}MA:{d[f'{short_ma}MA']:.2f}")
    ...
    if uploaded_file is not None:
        try:
            ...
            if st.button("🚨 呈交給 AI 大師進行「毒舌體質診斷」", type="primary"):
                if not user_api_key: st.error("🚨 記得先在左側欄位輸入您的 API Key 喔！")
                else:
...
with tab_chat:
    ...
    if prompt := st.chat_input("請問大師... (例如：請問現在台積電的法說會預期好嗎？)"):
        if not user_api_key: st.error("🚨 記得先輸入 API Key 才能開啟對話喔！")
        else:
...
with tab_predict:
    ...
    if st.session_state.prediction_result:
        ...
        st.success(f"🤖 太棒了！系統已經從基本面報告中，為您鎖定 {len(st.session_state.predicted_tickers)} 檔潛力股代號：{st.session_state.predicted_tickers}")
        ...
        if not target_tickers:
            ...
            if scan_results:
                ...
                st.dataframe(df_stealth.style.applymap(
                    lambda x: 'background-color: #e6f7ff; color: #0050b3; font-weight: bold' if "嚴重低估" in str(x) else ('background-color: #ffcccc; color: black' if "籌碼進場" in str(x) else ''), 
                    subset=['綜合策略判定 (基本面+技術面)']
                ), hide_index=True, use_container_width=True)
                
                st.markdown(f"> **💡 總司令解讀指南**：\n> * 如果看到藍色的 **「💎 嚴重低估 (適合左側建倉)」**，代表這檔股票雖然現在跌破均線沒人要，但 AI 已經幫您確認過它的財報跟訂單非常棒！您可以開始慢慢往下撿便宜囉！\n> * 如果看到紅色的 **「🚨 籌碼進場」**，代表基本面的好消息已經開始有人偷偷買單了，適合大膽進場！")
            else:
